...
def train():
    with tf.Graph().as_default():
        ...
        for epoch in range(MAX_EPOCH):
            ...
            sys.stdout.flush()
            eval_one_epoch_stage_3(sess,ops,train_writer)
...
def eval_one_epoch_stage_3(sess, ops, train_writer):
    ...
    for i in range(47):
        ...
        pred_proposal = np.zeros((num_data, NUM_POINT,2), np.float32)
        pred_dof_regression = np.zeros((num_data, 1, 6), np.float32)
        for j in range(num_batch):
            ...
            summary, step,loss_val,loss1_val,loss2_val,acc_val,iou_val,pred_proposal[begin_idx:end_idx,:,:],pred_dof_regression[begin_idx:end_idx,:,:]=sess.run([ops['merged'], ops['step'],ops['loss'],ops['loss1'],ops['loss2'],ops['task_1_acc'],ops['iou'],ops['pred_proposal'],ops['pred_dof_regression']],feed_dict=feed_dict)
