...
filenum = cellfun(@(x)sscanf(x,'training_data_%d.mat'), filenames);
[~,Sidx] = sort(filenum);
...
% core_pred_regression_direction
...
score_thresh = 0.3;
while all(max_score < score_thresh)
    score_thresh = score_thresh-0.05;
end
disp(score_thresh);
score_id(max_score<score_thresh) = [];
s_mat(max_score<score_thresh,:)=[];
max_score(max_score<score_thresh) = [];
